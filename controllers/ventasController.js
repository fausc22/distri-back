...
const guardarPdfFactura = (cliente_id, cliente_nombre, cliente_telefono, cliente_direccion, cliente_ciudad, cliente_provincia, cliente_condicion, cliente_cuit, tipo_documento, tipo_fiscal, total, estado, empleado_id, empleado_nombre, venta_id, productos) => {
    ...
    try {
        ...
        htmlTemplate = htmlTemplate
            .replace("{{fecha}}", venta.fecha)
            .replace("{{cliente_nombre}}", cliente_nombre)
            .replace("{{cliente_cuit}}", cliente_cuit || "No informado")
            .replace("{{cliente_cativa}}", cliente_condicion || "No informado");
        ...
        page.setContent(htmlTemplate, { waitUntil: "networkidle0" }); // ⬅️ Espera hasta que la página cargue completamente
        const pdfBuffer = page.pdf({ format: "A4" });
...
const obtenerComprobante = (req, res) => {
    ...
    fs.readdir(comprobantesPath, (err, files) => {
        ...
        const comprobante = files.find(f => path.parse(f).name === ventaId);
        if (!comprobante) {
